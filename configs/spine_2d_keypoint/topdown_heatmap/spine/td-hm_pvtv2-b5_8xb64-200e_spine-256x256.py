...
model = dict(
    ...
    data_preprocessor=dict(
        ...
        bgr_to_rgb=True),
    backbone=dict(
        type='PyramidVisionTransformerV2',  # 改成V2版本
        embed_dims=64,
        num_layers=[3, 6, 40, 3],  # B5的层数配置
        num_heads=[1, 2, 5, 8],    
        mlp_ratios=[4, 4, 4, 4],   # B5的MLP比率
        drop_path_rate=0.3,        # B5推荐的drop path率
        init_cfg=dict(
            type='Pretrained',
            checkpoint='/root/task2/weight/pvt_v2_b5_22k_30ep.pth'  # 本地权重文件路径
        ),
    ),
